...
static unsigned new_swap_page()
{
    ...
    /*for (i = count * PAGESIZE; i < (count+1) * PAGESIZE; i++)
        swap[i] = 0;*/
...
static unsigned take_phys_page(unsigned virt_page)
{
    ...
    /*if (!page_table[virt_page].ondisk) {
        page_table[virt_page].page = new_swap_page();
        page_table[virt_page].ondisk = 1;
    }*/
    ...
    if (coremap[page].owner != NULL) {
        ...
        if (coremap[page].owner->modified) {
            ...
            print_pages(page, coremap[page].page);
    ...
    if (page_table[virt_page].ondisk) {
        ...
        read_page(page, coremap[page].page);
