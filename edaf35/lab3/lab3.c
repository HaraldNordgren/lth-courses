...
/*static list_t               fifo_queue;*/
static unsigned             next_memory = -1;
...
static void print_pages(unsigned ram, unsigned sw)
{
    unsigned    j;
    
    printf("  mem%d:\t\t", ram);
    for (j = 0; j < PAGESIZE; j++) {
        printf(" %x", memory[ram * PAGESIZE + j]);
    }
    
    if (sw != -1) {
        printf("\n  swap%d:\t", sw);
        for (j = 0; j < PAGESIZE; j++) {
            printf(" %x", swap[sw * PAGESIZE + j]);
        }
    }
...
static unsigned take_phys_page()
{
    ...
    coremap_entry_t     *ram_entry;
    /*page_table_entry_t  *swap_entry;*/

	page = (*replace)();
    
    ram_entry = &(coremap[page]);
    /*swap_entry = &(page_table[virt_page]);*/
    
    
    if (ram_entry->owner != NULL) {
        
        if (ram_entry->owner->modified) {
            if (!ram_entry->owner->ondisk) {
                ram_entry->page = new_swap_page();
                /*ram_entry->page = ram_entry->owner->page;*/
                
                /*ram_entry->owner->page = ram_entry->page;*/
                /* Gamla värdet på owner->page ? */
                /* a8d4 */

                ram_entry->owner->ondisk = 1;
            }
            
            ram_entry->owner->page = ram_entry->page;

            printf("before write_page\n");
            print_pages(page, ram_entry->page);

            write_page(page, ram_entry->page);
            ram_entry->owner->modified = 0;
            
            printf("after write_page\n");
            print_pages(page, ram_entry->page);
        } else {
            printf("Unmodified memory swapped");
        }

        printf("\n");
        ram_entry->owner->inmemory = 0;
    }
...
static void pagefault(unsigned virt_page)
{
    ...
    ram_entry = &(coremap[page]);
    vm_entry = &(page_table[virt_page]);
	
    ram_entry->owner = vm_entry;
    
    if (vm_entry->ondisk) {
        ram_entry->page = vm_entry->page;
    }
    ...
    if (vm_entry->ondisk) {
        printf("before read_page\n");
        print_pages(page, ram_entry->page);
        
        read_page(page, ram_entry->page);
        
        printf("after read_page\n");
        print_pages(page, ram_entry->page);
    } else {
        printf("New page\n");
        for (i = 0; i < PAGESIZE; i++) {
            memory[i + page * PAGESIZE] = 0;
        }
        print_pages(page, -1);
    }
    printf("\n");
    ...
    /*printf("swap%d =\t%d\n", page_table[virt_page].page,
            swap[page_table[virt_page].page * PAGESIZE]);*/
}
...
int run(int argc, char** argv)
{
	...
	while (proceed) {
		...
		case ST:
			...
			writeback = false;
            /*printf("dest_reg %x in %x\n", data, source1+constant);*/
			break;
